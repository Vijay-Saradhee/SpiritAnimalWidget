...
class AnimalRepository(context: Context) {
    ...
    fun getCurrent(): Animal {
        val id = prefs.getString("curr_id", "6")
        return animals.find { it.id == id } ?: animals.first()
    }

    fun pickRandom(): Animal {
        val current = getCurrent()
        val next = animals.filter { it.id != current.id }.random()
        prefs.edit().putString("curr_id", next.id).apply()
        return next
    }
}
